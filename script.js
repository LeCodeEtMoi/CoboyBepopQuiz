...
var currentQuestionIndex = 0; // Indice de la question courante
var score = 0; // Score
var chosenAnswer = null; // Variable pour stocker la réponse choisie
...
validationButton.addEventListener('click', function () {
    ...
    if (chosenAnswer !== null && isCorrectAnswer(currentQuestionIndex, chosenAnswer)) {
        ...
        console.log("La r\u00E9ponse ".concat(chosenAnswer + 1, " est incorrecte.")); // Affiche un message si la réponse est incorrecte
    }
    // Incrémente le numéro de la question
    currentQuestionIndex++;
    if (currentQuestionIndex < questionnaire.length) {
        displayQuestion(currentQuestionIndex); // Affiche la prochaine question
        questionElement.textContent = "".concat(currentQuestionIndex + 1); // Met à jour le numéro de la question affiché
    }
    else {
        // Affichage final avec "Bien joué !"
        var finalMessage = "Bien jou\u00E9 ! Votre score est de ".concat(score, " sur ").concat(questionnaire.length, ".");
        fin.textContent = "Votre score est de ".concat(score, " / ").concat(questionnaire.length); // Met à jour le score affiché
